...
from .views import session_detail, talk_frame, other_frame, preview

urlpatterns = [
    url('^$', RedirectView.as_view(pattern_name='talks_list')),
    url('^talk-frame/(?P<frame_type>(video-only|16-9|4-3|intro))/(?P<session_id>\\d+)$', talk_frame, name='talk_frame'),
    url('^other-frame/(?P<frame_type>(sponsors|pycon))$', other_frame, name='other_frame'),
